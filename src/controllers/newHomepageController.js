...
export const getHomepageData = asyncHandler(async (req, res) => {
  try {
    ...
    // 3. Get active homepage tags
    const homepageTags = await Tag.find({ isActive: true }).sort({ createdAt: -1 });
    ...
    // Process tag-based products
    const tagProducts = {};
    for (const tag of homepageTags) {
      const products = await Product.find({ tags: { $in: [tag.name] } })
        .populate("category")
        .sort({ createdAt: -1 })
        .limit(8);

      if (products.length > 0) {
        tagProducts[tag.name] = {
          tag: {
            _id: tag._id,
            name: tag.name,
            slug: tag.slug,
